...
public class TC_ProcessorFailour16 {
	String fileName = "RWS_C_ProcessorFailure_100.16";
	String transType = "02"; // Change accordingly
    String transName = "Refund Without Sale"; // Change accordingl y
	...
	List<String> headlines =  xlWriter.headlineSetter("99", "UNABLE TO SEND TRANSACTION TO BE AUTHORIZED BY ISSUER.", "CHASE");
		...
		try {
			...
			if (result.equalsIgnoreCase("Approved")) {
				...
				saleData.add(3, transName);
					...
					List<String> expectedList =  xlWriter.generateTransactionSteps("99", 0);
	...
	// There are 3 CREDIT OR DEBIT Sales (Using Amount 100.16)
	@Test(invocationCount = 3, priority = 5)
	public void F_creditSale() throws Exception, Exception {    
	
		
		System.out.println("PLEASE USE CREDIT CARD  :: This sale is using amount 100.16");
		...
		try {
			...
			if (result.equalsIgnoreCase("Approved")) {
				...
				List<String> expectedList =  xlWriter.generateTransactionSteps("99", invocationCount);
		...
		public void P_HardCode__FSA() throws Exception, Exception {
			...
			try {
				...
				String Sale_Request = FSA_Sale_Request.P_FSA_SALE_Request(
						GCBPrameter.getParameterValue("CardToken"), GCBPrameter.getParameterValue("CardIdentifier"),
						GCBPrameter.getParameterValue("CRMToken"), transType, 0.16);
...
@Test(invocationCount = 1 , priority = 7)
		public void P_HardCode__EBT() throws Exception {
			...
			try {
				...
				String Sale_Request = EBT_Sale_Request.P_EBT_SALE_REQUEST(GCBPrameter.getParameterValue("CardToken"), transType, "100.16");  
	...
	@Test(priority = 11, dataProvider = "VoidData", dataProviderClass = TC_ProcessorFailour16.class)
	...
	// Performed 2 CREDIT OR DEBIT transaction using 100.16 for check counter reset
	// or not

	@Test(invocationCount = 4, priority = 16)
	public void F_AfterReset_creditSale() throws Exception, Exception {
		System.out.println("PLEASE USE CREDIT CARD for the using amount 100.16 ");
		...
		try {
			...
			String Sale_Request = Sale_Request_Modification.P_SALE_REQUEST(
					GCBPrameter.getParameterValue("CardToken"), GCBPrameter.getParameterValue("CardIdentifier"),
					GCBPrameter.getParameterValue("CRMToken"), transType, "100.16");
