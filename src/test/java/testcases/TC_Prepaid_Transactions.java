...
public class TC_Prepaid_Transactions {
	...
	public void test_Gift_Manual_CI(String transactionType, String amount, String cardNumber, String entrySource,
		...
		if (upsdata.isEmpty()) { 
			...
			System.out.println("Please Enter dynamic UPC Data ");
			System.out.println("_______________________________________");
			upsdata = sc.next();
		}
		...
		try {
			...
			if (result.equalsIgnoreCase("Approved") && transactionType.contains("Pre-Auth activation") || transactionType.contains("Pre-Auth Reload"))  {

				String Gift_PreAuthActivationreq = Prepaid_Request_Modification.modified_Prepaid_Request(amount, null, null, subtransType, transtype, upsdata, GCBPrameter.getParameterValue("CardToken") );
			...
			//	System.out.println(Gift_PreAuthActivationreq);
				...
				String Gift_PreAuthActivationreq = Prepaid_Request_Modification.modified_Prepaid_Request2(amount, null, null,	subtransType, transtype, upsdata, GCBPrameter.getParameterValue("CardIdentifier") );
				cp.sendRequestToAESDK(Gift_PreAuthActivationreq);
			//	System.out.println(Gift_PreAuthActivationreq);
				String response_PreAuthActivation = cp.receiveResponseFromAESDK();
				Response_Parameters giftparameters = new Response_Parameters(response_PreAuthActivation);
	...
	public void test_Gift_Manual_SWipe(String transactionType, String amount, String cardNumber, String entrySource,
		...
		try {
			...
			if (result.equalsIgnoreCase("Approved") && transactionType.contains("Pre-Auth activation") || transactionType.contains("Pre-Auth Reload"))  {
				...
				String Gift_PreAuthActivationreq = Prepaid_Request_Modification.modified_Prepaid_Request2(amount, null, null,
						subtransType, transtype, upsdata, GCBPrameter.getParameterValue("CardIdentifier") );
				cp.sendRequestToAESDK(Gift_PreAuthActivationreq);
			//	System.out.println(Gift_PreAuthActivationreq);
				String response_PreAuthActivation = cp.receiveResponseFromAESDK();
				Response_Parameters giftparameters = new Response_Parameters(response_PreAuthActivation);
