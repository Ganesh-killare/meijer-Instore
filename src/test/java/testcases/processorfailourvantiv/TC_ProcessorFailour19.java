...
public class TC_ProcessorFailour19 {
	String fileName = "W_ProcessorFailure_100.19";
	...
	List<String> headlines =  xlWriter.headlineSetter("96", "SYSTEM ERROR", "WORLDPAY");
		...
		try {
			...
			if (result.equalsIgnoreCase("Approved")) {
				...
				saleData.add(3, transName);
					...
					List<String> expectedList =  xlWriter.generateTransactionSteps("96", 0);
	...
	// There are 3 CREDIT OR DEBIT Sales (Using Amount 100.19)
	@Test(invocationCount = 3, priority = 5)
	public void F_creditSale() throws Exception, Exception {    
	
		
		System.out.println("PLEASE USE CREDIT CARD  :: This sale is using amount 100.19");
		...
		try {
			...
			if (result.equalsIgnoreCase("Approved")) {
				...
				List<String> expectedList =  xlWriter.generateTransactionSteps("96", invocationCount);
	...
	// One FSA Transaction using amount 100.19
	...
	// There are 1 FSA Sales (After Failour transactions should be go through wordpay)
		...
		public void P_HardCode__FSA() throws Exception, Exception {
			System.out.println("PLEASE USE FSA CARD  :: This sale is using amount 100.19");
			...
			try {
				...
				String Sale_Request = FSA_Sale_Request.P_FSA_SALE_Request(
						GCBPrameter.getParameterValue("CardToken"), GCBPrameter.getParameterValue("CardIdentifier"),
						GCBPrameter.getParameterValue("CRMToken"), transType, 0.19);
		...
		// Two EBT Transactions Using 100.19 amount
		
		
@Test(invocationCount = 1 , priority = 7)
		public void P_HardCode__EBT() throws Exception {
			System.out.println("PLEASE USE EBT CARD  :: This sale is using amount 100.19");      
			...
			try {
				...
				String Sale_Request = EBT_Sale_Request.P_EBT_SALE_REQUEST(GCBPrameter.getParameterValue("CardToken"), transType, "100.19");  
	...
	@Test(priority = 11, dataProvider = "VoidData", dataProviderClass = TC_ProcessorFailour19.class)
	...
	// Performed 2 CREDIT OR DEBIT transaction using 100.19 for check counter reset
	// or not

	@Test(invocationCount = 4, priority = 16)
	public void F_AfterReset_creditSale() throws Exception, Exception {
		System.out.println("PLEASE USE CREDIT CARD for the using amount 100.19 ");
		...
		try {
			...
			String Sale_Request = Sale_Request_Modification.P_SALE_REQUEST(
					GCBPrameter.getParameterValue("CardToken"), GCBPrameter.getParameterValue("CardIdentifier"),
					GCBPrameter.getParameterValue("CRMToken"),transType, "100.19");
