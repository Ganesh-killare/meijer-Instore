...
public class TC_ProcessorFailour21 {
	String fileName = "W_ProcessorFailure_100.21";
	...
	List<String> headlines =  xlWriter.headlineSetter(" NO ", "ISSUER OR SWITCH INOPERATIVE (VISA) ", "WORLDPAY");
		...
		try {
			...
			if (result.equalsIgnoreCase("Approved")) {
				...
				saleData.add(3, transName);
					...
					List<String> expectedList =  xlWriter.generateTransactionSteps("NO", 0);
	...
	// There are 3 CREDIT OR DEBIT Sales (Using Amount 100.21)
	@Test(invocationCount = 5, priority = 5)
	public void F_creditSale() throws Exception, Exception {    
	
		
		System.out.println("PLEASE USE ONLY VISA CARD  :: This sale is using amount 100.21");
		...
		try {
			...
			if (result.equalsIgnoreCase("Approved")) {
				...
				List<String> expectedList =  xlWriter.generateTransactionSteps("NO", invocationCount);
	...
	// One FSA Transaction using amount 100.21
...
//		@Test(priority = 6)
		public void P_HardCode__FSA() throws Exception, Exception {
			System.out.println("PLEASE USE FSA CARD  :: This sale is using amount 100.21");
			...
			try {
				...
				String Sale_Request = FSA_Sale_Request.P_FSA_SALE_Request(
						GCBPrameter.getParameterValue("CardToken"), GCBPrameter.getParameterValue("CardIdentifier"),
						GCBPrameter.getParameterValue("CRMToken"), transType, 0.21);
		...
		// Two EBT Transactions Using 100.21 amount
		
		
// @Test(invocationCount = 2 , priority = 7)
		public void P_HardCode__EBT() throws Exception {
			System.out.println("PLEASE USE EBT CARD  :: This sale is using amount 100.21");      
			...
			try {
				...
				String Sale_Request = EBT_Sale_Request.P_EBT_SALE_REQUEST(GCBPrameter.getParameterValue("CardToken"), transType, "100.21");  
	...
	@Test(priority = 11, dataProvider = "VoidData", dataProviderClass = TC_ProcessorFailour21.class)
	...
	// Performed 2 CREDIT OR DEBIT transaction using 100.21 for check counter reset
	// or not

	@Test(invocationCount = 4, priority = 16)
	public void F_AfterReset_creditSale() throws Exception, Exception {
		System.out.println("PLEASE USE CREDIT CARD for the using amount 100.21 ");
		...
		try {
			...
			String Sale_Request = Sale_Request_Modification.P_SALE_REQUEST(
					GCBPrameter.getParameterValue("CardToken"), GCBPrameter.getParameterValue("CardIdentifier"),
					GCBPrameter.getParameterValue("CRMToken"),transType, "100.21");
