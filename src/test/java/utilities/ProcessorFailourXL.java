...
public class ProcessorFailourXL {
	...
	public List<String> generateTransactionSteps(String responseCode, int counter) {
	    ...
	    steps.add("Transaction should get routed to primary, and should get declined with   " + responseCode + "   response and host should rout that transaction to Secondary  and it should get approved. Host will set the Primary failover counter to " + counter);
	    steps.add("Transaction should get routed to secondary directly as counter limit is exceeded and should get approved as hardcoded amount is not used");
	    steps.add("Transaction should get routed to primary, and should get approved as hardcoded amount is not used");
	    steps.add("As we have performed approved transaction before the threshold for failover counter was exceeded, failover counter should get reset to zero.");
	    ...
	    steps.add("Performed void transactions for the transactions performed through Worldpay and chase processor, all the void transactions should get routed to their respective processor where parent transaction was approved");
	    steps.add("Here, as we have reset the counter, all the transactions should start getting routed to Primary processor again. Transactions should get approved from chase as we have not used hardcoded amounts.");
	  

	
	    
	    steps.add("As we have performed 4 failover and our threshold for failover is 5 counters, we will now perform the approval transaction to check if the counter that is set for above failovers is getting reset to zero again.");
	  ...
	  public List<String> headlineSetter(String Response , String responseText, String primaryProcesssor ){
		  ...
		  headlines.add("Transaction performed using 45.00 $ for the reset the counter");
		  headlines.add("Dual processor testing chase and WorldPay (where "+ primaryProcesssor + "  is primary and processor response code tested is "+ Response+ " response text is  " +responseText);
		  headlines.add("Failover is now done and all the transactions going forward will get route to Secondary processor directly for the particular tender is done processor failure ");
		  headlines.add("Reset the Failover counter to 0 and perform basic sale transactions without hardcoded amount.");
		  headlines.add("To check If counter is getting reset to zero, when performed failover 4 time and before the failover threshold approved transaction is done");
     return headlines ; 
	  }
    ...
    public ProcessorFailourXL() {
        ...
        parameters = Arrays.asList("Expected Steps ","CardToken", "CardIdentifier", "CRMToken","TransactionType", "CardEntryMode", "TransactionTypeCode",
                "TransactionSequenceNumber", "CardType", "SubCardType", "TotalApprovedAmount", "ResponseText",
                "ResponseCode", "TransactionIdentifier", "AurusPayTicketNum", "ApprovalCode","Primary processor Failover counter", "ProcessorMerchantId", "NOTES");
