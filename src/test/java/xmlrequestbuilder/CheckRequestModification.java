...
public class CheckRequestModification {
	...
	public static String modified_Check_Request( ) {
		...
		double Total = cashback +tender ;
		String cashbackAmount = df.format(cashback);
		String tenderamount = df.format(tender);
		String transactionTotal = df.format(Total);
		...
		try {
			...
			root.getChild("TransAmountDetails").getChild("CashBackAmount").setText(cashbackAmount);
			root.getChild("TransAmountDetails").getChild("TenderAmount").setText(tenderamount);
			root.getChild("TransAmountDetails").getChild("TransactionTotal").setText(transactionTotal);
